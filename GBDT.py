...
def splitData(data_array, col, value): #选定分割点之后

    array_1 = data_array[data_array[:, col] >= value, :] #col大于等于value分给array_1
...
def get_best_split(data_array, ops=(1, 4)):  #GBDT划分，最小平方误差
    tolS = ops[0];
    tolN = ops[1]
    ...
    best_col = 0;
    best_value = 0;
    S = getErr(data_array) #当方差最小化不明显时的一个判定
    for col in xrange(n - 1):
        values = set(data_array[:, col])
        for value in values:
            array_1, array_2 = splitData(data_array, col, value)
            if (array_1.shape[0] < tolN) or (array_2.shape[0] < tolN): continue  #划分的太小
...
def gbdt(data_array, num_iter, ops=(1, 4)):
    ...
    for i in xrange(num_iter):
        ...
        if i == 0:
            ...
            r = y - np.array(yHat) #实际值与预测值之间的差值  残差
            data_array = np.hstack((x, r))
            tree = buildTree(data_array, ops)
            list_trees.append(tree)
            rHat = createForeCast(tree, x)
            yHat = yHat + rHat  #预测值等于累加
    return list_trees
